...
void getEstadoRuleta(int &numjug, int &sumdin, int &numlanzbol, int &gananciabanca);
{
	numjug = (int)jugadores_.size();
	sumdin = banca_;
	numlanzbol = numlanzbol_;
	gananciabanca = gananciabanca;
	
	list <Jugador>::iterator it; 

	for(it = jugadores_.begin(); it != jugadores_.end(); it++)
	{
		sumdin = sumdin + it->gerDinero();
	}
}
